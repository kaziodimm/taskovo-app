...
export default async function TaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <Header />
      <main className="page-shell task-detail-shell">
        <section className="page-hero task-detail-hero">
          <div>
            <p className="kicker">Detail objednávky</p>
            <h1 className="page-title">{task.title}</h1>
            <p className="hero-lead">{task.description}</p>
            <div className="task-meta detail-meta"><span>{task.category}</span><span>{task.city}</span>{task.district ? <span>{task.district}</span> : null}<span>{task.desired_time}</span></div>
          </div>
          <div className="page-hero-card task-summary-card">
        ...
        <section className="order-grid">
          <div className="order-main">
            <section className="order-panel">
              ...
              {isClientOwner ? (
                <form className="compact-form attachment-form" action={addTaskAttachment}>
          ...
          <aside className="order-sidebar">
            <section className="order-panel sticky-panel">
              ...
              {acceptedOffer ? (
                <div className="selected-tasker-box">
